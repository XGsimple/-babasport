...
public class OrderServiceImpl implements OrderService {
	...
	public Integer addOrder(Order order,BuyCart buyCart) {
		//生成订单号
		DateFormat dFormat = new SimpleDateFormat("yyyyMMddHHmmssSSSS");
		String oid = dFormat.format(new Date());
		order.setOid(oid);
		//运费
		...
		//支付状态  PaymentWay 0：到付  1：在线 2：邮局 3：公司转账
		...
		//保存订单，同时获取到数据自动生成的id，赋值到order的id属性上（通过selectKey）
