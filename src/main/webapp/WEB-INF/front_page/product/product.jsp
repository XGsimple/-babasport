				...
				<c:if test = "${flag}">
					<li><label>已选条件：</label>
						<p class="sel">
							...
							<a href="javascript:void(0);"> <em>${q.key }</em>${q.value }
			...
			<div class="mt ofc">
				<ul class="uls i_150x150 x4_150x150b">
					...
					<!-- 图片链接为静态页，文字链接为动态页 -->
						...
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="javascript:void(0)" onclick="window.open('/product/detail.shtml?id=${entry.id }')"  title="${entry.name }" target="_blank">${entry.name }</a></dt>
							<!-- dt 25个文字+... -->
							<dd class="h40">${entry.name }</dd>
							<dd class="orange">￥128.00</dd>
							<dd>北京有货</dd>
							<dd><a href="javascript:void(0);"  title="加入购物车" class="inb btn70x21 mr">加入购物车</a></dd>
						</dl>
